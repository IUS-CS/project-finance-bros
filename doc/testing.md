...
For unit tests, we utilized Django's test.py file. This can be run with the "python manage.py runserver" command. We included tests for entering, editing, and deleting transactions. Additionally, we included tests for uploading PDF statements. There are also edge case tests for checking that the action will fail if the user inputs incorrectly.
